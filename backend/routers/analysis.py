...
@router.post("/sprint/{sprint_id}/quality")
async def quality_analysis(sprint_id: int, board_id: int = None):
    """LLM 기반 품질 이슈 분석"""
    try:
        raw = await jira_service.fetch_sprint_issues(sprint_id)
        issues = [jira_service.normalize_issue(i) for i in raw]
        ...
        if board_id:
            ...
            for s in sprints:
                if s["id"] == sprint_id:
                    ...
                    break
        stats = jira_service.compute_sprint_stats(sprint_info, issues)
        quality = await ai_service.analyze_quality(issues, stats)
        return {"quality": quality, "stats": stats, "sprint_id": sprint_id}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
