...
# ── 1단계 예측 (Burndown · Velocity · EMV · 자원) ─────────────────────────────


def _fc_status_from_delay(delay_days: float) -> str:
    if delay_days <= 0:
        return "ok"
    if delay_days <= 2:
        return "warning"
...
def _fc_status_from_pct(pct: float, warn: float = 70, crit: float = 85) -> str:
    if pct < warn:
        return "ok"
    if pct < crit:
        return "warning"
...
def _burndown_forecast(burndown: dict, summary: dict) -> dict[str, Any]:
    ...
    status = _fc_status_from_delay(delay_days)
    if remaining <= 0:
        status = "ok"
...
def _emv_forecast(risk_data: dict | None) -> dict[str, Any]:
    ...
    status = _fc_status_from_pct(reserve_pct)
...
async def get_sprint_plan_forecast() -> dict[str, Any]:
    ...
    alerts = _build_forecast_alerts(burndown_fc, velocity_fc, emv_fc, resource_fc)
